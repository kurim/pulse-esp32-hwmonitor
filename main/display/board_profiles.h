...
typedef enum {
    ...
    DISPLAY_GUITION_JC8048W550, // Guition JC8048W550(C): ST7262 800x480 RGB-Parallel + GT911-Touch
                                // (Werksverdrahtung, nur auf ESP32-S3 verfuegbar, siehe
                                // board_profile_is_available())
...
typedef enum {
    LCD_BUS_SPI,
    LCD_BUS_I2C,
    LCD_BUS_RGB, // 16-bit RGB565-Parallelbus (esp_lcd_new_rgb_panel), nur ESP32-S3
} lcd_bus_kind_t;
...
typedef enum {
    ...
    LCD_SHAPE_WIDE,   // Dashboard-UI fuer grosse Panels, Verlaufsgrafen direkt auf der
                      // Hauptseite statt eigener Detail-Unterseiten (build_wide_ui)
} lcd_ui_shape_t;
...
typedef struct {
    ...
    // ---- I2C-Displays ----
    // Bei LCD_BUS_RGB (kein eigener Display-I2C-Bus) werden dieselben drei
    // Felder stattdessen fuer den I2C-Bus des GT911-Touch-Controllers
    // genutzt (siehe touch_gt911.c) - spart eigene sda/scl/addr-Felder nur
    // fuers Touch-IC.
    int i2c_sda, i2c_scl;
    int i2c_addr;
    int i2c_hz;
    ...
    // ---- Touch (I2C: nur LCD_BUS_RGB/GT911) - RESET/INT-Pins, I2C-Bus
    // siehe i2c_sda/i2c_scl/i2c_addr oben.
    int touch_rst, touch_int;

    // ---- RGB-Parallelbus (nur LCD_BUS_RGB) ----
    int rgb_data[16];    // D0..D15 (Bit-Reihenfolge wie von esp_lcd_new_rgb_panel erwartet)
...
typedef struct {
    ...
    int16_t nav_button;
    // GT911-Touch (LCD_BUS_RGB) - RESET/INT. Der 20-Pin-RGB-Datenbus selbst
    // (siehe board_profile_t.rgb_data/rgb_hsync/...) ist bewusst NICHT
    // ueberschreibbar: anders als die generischen SPI-Profile (fliegende
    // Verdrahtung auf einem Devboard) ist das Guition-Board ein fertiges,
    // werksseitig verdrahtetes Komplettmodul - 20 zusaetzliche Override-Felder
    // dafuer haetten kaum praktischen Nutzen.
