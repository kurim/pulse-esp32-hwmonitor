...
volatile uint32_t hw_rx_count = 0;

static SemaphoreHandle_t s_hw_data_mutex = xSemaphoreCreateMutex();

void hw_data_lock(void)   { xSemaphoreTake(s_hw_data_mutex, portMAX_DELAY); }
void hw_data_unlock(void) { xSemaphoreGive(s_hw_data_mutex); }
...
void config_set_defaults(app_config_t *cfg)
{
    ...
    // die CYD relevant.
    cfg->touch_x_min = 462;
    cfg->touch_x_max = 3491;
    cfg->touch_y_min = 540;
    cfg->touch_y_max = 3417;
    cfg->touch_calibrated = false;
}
