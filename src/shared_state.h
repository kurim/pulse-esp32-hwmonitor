...
#define FW_VERSION "2.4.0-arduino"
...
// direkt per USB/Serial vom PC-Client (siehe src/net/serial_handler.cpp).
...
typedef struct {
    ...
    // Sekunden ohne neue Hardwaredaten, bevor das Display in den Standby
    ...
    // Pin-Overrides fuer das aktuell gewaehlte Display.
    pin_override_t pin_overrides;
} app_config_t;

// ----------------------------------------------------------------
// Live-Hardwarewerte (per MQTT/USB vom PC-Client geliefert)
// ----------------------------------------------------------------
typedef struct {
...
typedef struct {
    ...
    float   feels_like_c;
    int     humidity;      // %
    float   wind_speed;    // km/h (metric) bzw. mph (imperial)
...
// Globale Instanzen (definiert in shared_state.cpp)
...
extern volatile bool  serial_connected;  // true nach der ersten gueltigen Zeile per USB/Serial

// Millisekunden seit Boot.
int64_t now_ms(void);
